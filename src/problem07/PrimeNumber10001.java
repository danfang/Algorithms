...
public class PrimeNumber10001 {
   ...
   public static void main(String[] args) {
      ...
      while (true) {
         if (printedInstructions == false) {
            ...
            System.out.println();
            System.out
                  .println("Type 't' to test for primality, or 'f' to find a prime number.");
   ...
   public static void findPrime(int numberOfPrimes) {
      int primeCount = 1, testNumber = 1; // primeCount starts at 1, because 2 is not
                                          // tested
      while (primeCount < numberOfPrimes) {
         boolean primeNumber = false;
         boolean isDivisible = false;
         // if a prime number is found OR a number is proven to be non-prime, exit
         while (primeNumber == false || isDivisible == false) {
            int root = (int) Math.ceil(Math.sqrt(testNumber));
            // divides testNumber by (2) through (testNumber - 1)
            for (int i = 2; i < root + 1; i++) {
               if ((double) testNumber % i == 0) {
                  // testNumber is proven to be a non-prime number, primeCount is NOT
                  // incremented
                  isDivisible = true;
                  i = testNumber; // ends the for loop
               } else if (i == root) {
                  primeNumber = true; // testNumber is proven to be a prime number
                  primeCount++;
               }
            }
      ...
      System.out.println(testNumber + " is the " + numberOfPrimes
